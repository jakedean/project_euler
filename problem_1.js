...
jakeUtil = require('./utility/js_utility_library.js');

// I will pass the max into node from the command line and grab them
// through process.argv.  In this case it will be 1000.

var max = jakeUtil.toNumber(process.argv[2]);

// Now we will increment through the range of multiples of 3 and
// add them to the arrayOfMultiples.
var arrayOfMultiplesOfThree = jakeUtil.range(max, 3, 3);

// Now we will do the same for the multiples of 5.
var arrayOfMultiplesOfFive = jakeUtil.range(max, 5, 5);
...
var uniqueMultiples = jakeUtil.unique(arrayOfMultiplesOfThree.concat(arrayOfMultiplesOfFive), true, true);

// Now we will reduce this down to one number.
var answer = jakeUtil.sum(uniqueMultiples);

// Got it, show the user!
